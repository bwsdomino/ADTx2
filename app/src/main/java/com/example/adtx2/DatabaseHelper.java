...
public class DatabaseHelper extends SQLiteOpenHelper{
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String createTable2="CREATE TABLE " + OBJ_TABLE + " (" + COLUMN_OBJ_ID + " INTEGER PRIMARY KEY AUTOINCREMENT, " + COLUMN_OBJ_NAME + " TEXT," + COLUMN_OBJ_SIZE + " INTEGER, "+COLUMN_OBJ_ITEMS+" TEXT)";
    ...
    public boolean addOneObj(Obj_Model obj_model){
     ...
     public Item_Model getOne(String s){
            String queryString = "SELECT * FROM "+ITEM_TABLE+" WHERE "+COLUMN_ID+ " = " +s;
            SQLiteDatabase db=this.getReadableDatabase();
            Cursor cursor = db.rawQuery(queryString,null);
            ...
            if(cursor.moveToFirst()){
                ...
                String ItemName=cursor.getString(1);
                int ItemPrice=cursor.getInt(2);
                item_model = new Item_Model(ItemId,ItemName,ItemPrice);
                return item_model;
            }
            else{}
            cursor.close();
    ...
    public Obj_Model getOneObj(String s){
        String queryString = "SELECT * FROM "+OBJ_TABLE+" WHERE "+COLUMN_OBJ_ID+ " = " +s;
        SQLiteDatabase db=this.getReadableDatabase();
        Cursor cursor = db.rawQuery(queryString,null);
        ...
        if(cursor.moveToFirst()){
            ...
            obj_model = new Obj_Model(Objid,ObjName,ObjSize,ObjItems);
            return obj_model;
        }
        else{}
        cursor.close();
    ...
    public List<Obj_Model> getEveryObj(){
        ...
        String queryString = "SELECT * FROM "+OBJ_TABLE;
        SQLiteDatabase db=this.getReadableDatabase();
        Cursor cursor = db.rawQuery(queryString,null);
        if(cursor.moveToFirst()){
            do{
        ...
        SQLiteDatabase db=this.getWritableDatabase();
        String queryString = "DELETE FROM "+OBJ_TABLE+ " WHERE "+COLUMN_OBJ_ID+" = "+obj_model.getId();
        Cursor cursor = db.rawQuery(queryString, null);
        if(cursor.moveToFirst()){
            return true;
        }
    ...
    public boolean modOneObj(Obj_Model obj_model){
        SQLiteDatabase db=this.getWritableDatabase();
        String queryString = "UPDATE "+OBJ_TABLE+" SET "+COLUMN_OBJ_NAME+ " = " +'"'+obj_model.getName()+'"'+", "+ COLUMN_OBJ_SIZE +" = "+'"'+obj_model.getSize()+'"'+","+COLUMN_OBJ_ITEMS+" = "+'"'+obj_model.getItems()+'"'+" WHERE "+COLUMN_OBJ_ID+ "="+obj_model.getId();
        Cursor cursor = db.rawQuery(queryString,null);
        if(cursor.moveToFirst()){
            return false;
        }
